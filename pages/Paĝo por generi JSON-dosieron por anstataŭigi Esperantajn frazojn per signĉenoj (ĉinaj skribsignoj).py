...
#=====================================================================
st.set_page_config(
    page_title="Ilo por generi JSON-dosieron por la anstataŭigo de (ĉinaj ideogramoj) en Esperantaj frazoj",
    layout="wide"
)
st.title("Krei JSON-dosieron por anstataŭigi (ĉinajn ideogramojn) en Esperantaj frazoj.")
st.write("---")

#=====================================================================
# 2) 概要説明 (使い方)
#=====================================================================
with st.expander("Malfermi la klarigon pri la uzo", expanded=True):
    st.markdown("""
    #### Enkonduko
    En ĉi tiu paĝo eblas generi kaj elŝuti
    la JSON-dosieron (kiu eĉ povas esti ĉirkaŭ 50MB)
    por la fina anstataŭigo de Esperantaj frazoj en la ĉefa paĝo.

    **Mallonga gvidlinio**:
    1. Alŝutu (aŭ uzu defaŭltan) **dosieron CSV** (listo de korespondo inter
       radikoj de Esperanto kaj anglaj tradukoj, ktp).
    2. Se necese, alŝutu (aŭ uzu defaŭltan) **dosieron JSON**
       (reguloj pri radika segmentado kaj pri anstataŭigotaj tekstoj).
    3. Alklaku “Krei la JSON-dosieron por anstataŭigo” por
       **elŝuti** la finpretigitan JSON-dosieron.

    Malsupre troviĝas specimenaj dosieroj,
    kiujn vi povas elŝuti kaj rigardi por ekzemplaj formatoj
    aŭ personigita agordo.
    """)
...
# ダウンロードするボタンを設けています。
#=====================================================================
with st.expander("Listo de specimenaj dosieroj (por elŝuti)"):
    st.write("#### Specimenaj dosieroj")

    # サンプルCSV１
    st.markdown("""
    **Specimena CSV n-ro 1 (Korespondo inter Esperanto-radikoj kaj Angla-traduka rubio)**  
    Ĉi tiu CSV-listo enhavas unu-linian korespondon inter radikoj de Esperanto kaj angla traduko (kun rubio).
    Konstruu CSV en ĉi tiu formato kaj alŝutu ĝin,
    por ke ĉi tiu ilo generu la JSON-dosieron por anstataŭigo.
    """)
    ...
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Elŝuti Specimenan CSV n-ro 1 (Esperanto-radikoj & Angla-traduka rubio)",
            data=file,
            file_name="EsperantajRadikoj_AnglaTradukoRubio.csv",  # ★ファイル名をエスペラント化
            mime="text/csv"
        )

    # サンプルCSV２
    st.markdown("""
    **Specimena CSV n-ro 2 (Korespondo inter Esperanto-radikoj kaj ĉinaj ideogramoj, propono de Mingeo)**  
    Ĉi tiu CSV-listo montras kiel radikoj de Esperanto estas ligitaj al ĉinaj ideogramoj.
    """)
    file_path0 = './Appの运行に使用する各类文件/Mingeo先生版 世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Elŝuti Specimenan CSV n-ro 2 (Radikoj de Esperanto & ĉinaj ideogramoj - Propono Mingeo)",
            data=file,
            file_name="EsperantajRadikoj_ĈinajIdeogramoj_MingeoPropono.csv",  # ★ファイル名をエスペラント化
            mime="text/csv"
        )

    # サンプルCSV３
    st.markdown("""
    **Specimena CSV n-ro 3 (Korespondo inter Esperanto-radikoj kaj ĉinaj ideogramoj)**  
    Alia CSV-listo pri la rilato inter Esperanto-radikoj kaj ĉinaj ideogramoj.
    """)
    file_path0 = './Appの运行に使用する各类文件/世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Elŝuti Specimenan CSV n-ro 3 (Radikoj de Esperanto & ĉinaj ideogramoj)",
            data=file,
            file_name="EsperantajRadikoj_ĈinajIdeogramoj.csv",  # ★ファイル名をエスペラント化
            mime="text/csv"
        )

    # サンプルJSON１
    st.markdown("""
    **Specimena JSON n-ro 1 (Uzantaj agordoj pri segmentado de Esperantaj radikoj)**  
    **Uzo**: Vi povas precizigi kiel segmenti Esperantajn vortojn je radikoj,
    kiam aldoni verbajn finaĵojn, ktp.
    Rigardu la komentojn en la specimena dosiero por detaloj.
    (ekz. `["am", "dflt", ["verbo_s1"]]`)
    """)
    ...
    with open(json_file_path, "rb") as file_json:
        btn_json = st.download_button(
            label="Elŝuti Specimenan JSON n-ro 1 (Segmentado de Esperanto-radikoj)",
            data=file_json,
            file_name="UzantajAgordoj_SegmentadoDeEsperantajRadikoj.json",  # ★ファイル名をエスペラント化
            mime="application/json"
        )

    # サンプルJSON２
    st.markdown("""
    **Specimena JSON n-ro 2 (Uzantaj agordoj pri la finaj anstataŭigotaj tekstoj)**  
    **Uzo**: Se vi bezonas asigni apartajn ĉinajn ideogramojn aŭ specialajn formojn
    al certaj vortoj (krom la supre menciitaj segmentadaj reguloj),
    vi povas uzi ĉi tiun JSON-dosieron.
    (Plej ofte sufiĉas redakti la CSV kaj/aŭ la segmentadajn regulojn, do
     ĉi tiu dosiero ne estas ĉiam nepra.)
    """)
    json_file_path2 = './Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json'
    with open(json_file_path2, "rb") as file_json:
        btn_json = st.download_button(
            label="Elŝuti Specimenan JSON n-ro 2 (Uzantaj agordoj pri la anstataŭigita teksto)",
            data=file_json,
            file_name="UzantajAgordoj_AnstataŭigitaTeksto.json",  # ★ファイル名をエスペラント化
            mime="application/json"
        )
    ...
    **Specimena Excel n-ro 1 (14-lingva listo de Esperantaj radikoj & tradukaj rubioj)**  
    ...
    st.markdown("""
    **Specimena Excel n-ro 2 (Esperantaj radikoj – japana traduka rubio, kun nivelo de lernado)**  
    **Uzo**: Se vi deziras personigi kiujn Esperantajn radikojn havi kun traduka rubio,
    jen Excel-dosiero kiu enhavas ankaŭ "nivelon de lernado" bazitan sur
    baza Esperanto-japana vortaro.
    """)
    with open('./Appの运行に使用する各类文件/エスペラント語根-日本語訳ルビ対応リスト(習得レベル付き).xlsx', "rb") as file:
        st.download_button(
            label="Elŝuti Specimenan Excel n-ro 2 (Esperantaj radikoj – japana traduko kun lernnivelo)",
            data=file,
            file_name="EsperantajRadikoj_JapanaTradukoKunLernnivelo.xlsx",  # ★ファイル名をエスペラント化
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
...
# (ルビを親文字にして訳をrtタグに入れるか、逆に汉字を親文字にしてエスペラントをrtにするかなど。)
#=====================================================================
options = {
    'HTML-formato kun adaptita grandeco de la rubia teksto': 'HTML格式_Ruby文字_大小调整',
    'HTML-formato kun adaptita rubia teksto (ĉinaj ideogramoj anstataŭigitaj)': 'HTML格式_Ruby文字_大小调整_汉字替换',
    'HTML-formato': 'HTML格式',
    'HTML-formato (kun anstataŭigo de ĉinaj ideogramoj)': 'HTML格式_汉字替换',
    'Formo per krampoj': '括弧(号)格式',
    'Formo per krampoj (kun anstataŭigo de ĉinaj ideogramoj)': '括弧(号)格式_汉字替换',
    'Simpla anstataŭigo': '替换后文字列のみ(仅)保留(简单替换)'
}

display_options = list(options.keys())
selected_display = st.selectbox('Elektu elirformaton:', display_options)
...
#=====================================================================
st.header("Paŝo 1: Pretigi la CSV-dosieron")
st.markdown(
    """
### Bonvolu elekti **CSV-dosieron** kiu enhavas Esperantajn radikojn kaj tradukojn (ĉinaj ideogramoj ktp)

---
    """
)

csv_choice = st.radio("Kion fari kun la CSV-dosiero?", ("Alŝuti", "Uzi la defaŭlton"))
csv_path_default = "./Appの运行に使用する各类文件/Esperanto Roots - English Translation Ruby Correspondence List.csv"

CSV_data_imported = None

if csv_choice == "Alŝuti":
    st.write("Bonvolu alŝuti iun ajn CSV-dosieron. (UTF-8 rekomendita)")
    uploaded_file = st.file_uploader("Elektu la CSV-dosieron", type=['csv'])
    if uploaded_file is not None:
        ...
        st.success("CSV-dosiero sukcese alŝutita.")
    else:
        st.warning("Neniu CSV-dosiero alŝutita.")
        st.stop()

elif csv_choice == "Uzi la defaŭlton":
    try:
        ...
        csv_buffer = StringIO(converted_text)
        CSV_data_imported = pd.read_csv(csv_buffer, encoding="utf-8", usecols=[0, 1])
        st.info("Uzante la defaŭltan CSV-dosieron.")
    except FileNotFoundError:
        st.error("Ne eblas trovi la defaŭltan CSV-dosieron. Procezo haltigita.")
        st.stop()

st.write("Sukcese legis la CSV-dosieron. Ni iru al la sekva paŝo.")
st.write("---")

#=====================================================================
# ステップ2: JSONファイル(語根分解法など)を準備
#=====================================================================
st.header("Paŝo 2: Pretigi la(jn) JSON-dosieron(jn) (reguloj pri radikoj, ktp.)")
st.markdown("""
Eblas alŝuti aŭ uzi defaŭltajn JSON-dosierojn,
kie troviĝas **reguloj pri segmentado de Esperantaj radikoj** kaj/aŭ
**propraj anstataŭigaj tekstoj**.
Vi povas elŝuti specimenan JSON kaj ĝin redakti laŭbezone.
""")

# 1. エスペラント単語の語根分解法(カスタム)を指定する JSON
json_choice = st.radio("1. Kiel trakti la JSON-dosieron pri segmentado de Esperantaj radikoj?", ("Alŝuti", "Uzi la defaŭlton"))
json_path_default = "./Appの运行に使用する各类文件/世界语单词词根分解方法の使用者自定义设置.json"
custom_stemming_setting_list = None

if json_choice == "Alŝuti":
    uploaded_json = st.file_uploader("Bonvolu alŝuti la JSON-dosieron", type=['json'])
    if uploaded_json is not None:
        custom_stemming_setting_list = json.load(uploaded_json)
        st.success("JSON-dosiero sukcese alŝutita.")
    else:
        st.warning("Neniu JSON-dosiero alŝutita.")
        st.stop()
elif json_choice == "Uzi la defaŭlton":
    try:
        with open(json_path_default, "r", encoding="utf-8") as g:
            custom_stemming_setting_list = json.load(g)
        st.info("Uzante la defaŭltan JSON-dosieron.")
    except FileNotFoundError:
        st.error("Ne eblas trovi la defaŭltan JSON-dosieron.")
        st.stop()

# 2. 置換後文字列を追加指定する JSON
json_choice2 = st.radio("2. Kiel trakti la JSON-dosieron pri la finaj anstataŭigitaj tekstoj?", ("Alŝuti", "Uzi la defaŭlton"))
json_path_default2 = "./Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json"
user_replacement_item_setting_list = None

if json_choice2 == "Alŝuti":
    uploaded_json = st.file_uploader("Bonvolu alŝuti la duan JSON-dosieron", type=['json'])
    if uploaded_json is not None:
        ...
        st.warning("Neniu JSON-dosiero alŝutita.")
        st.stop()
elif json_choice2 == "Uzi la defaŭlton":
    try:
        with open(json_path_default2, "r", encoding="utf-8") as g:
            user_replacement_item_setting_list = json.load(g)
        st.info("Uzante la duan defaŭltan JSON-dosieron.")
    except FileNotFoundError:
        st.error("Ne eblas trovi la duan defaŭltan JSON-dosieron.")
        st.stop()
...
# ステップ3: 高度な設定 (並列処理)
#=====================================================================
st.header("Paŝo 3: Altnivelaj agordoj (paralela prilaborado)")
with st.expander("Malfermi agordojn pri la paralela prilaborado"):
    st.write("""
    Ĉi tie vi povas agordi la nombron de procezoj por la paralela
    prilaborado dum la konstruado de la anstataŭiga JSON-dosiero.
    Se la kvanto da teksto aŭ radikoj estas granda,
    uzi plurajn CPU-kernojn povas plirapidigi la procezon.
    """)

    use_parallel = st.checkbox("Uzi paralelan prilaboradon", value=False)
    num_processes = st.number_input("Kvanto de samtempaj procezoj", min_value=2, max_value=6, value=5, step=1)

st.write("### Krei la finan anstataŭigan JSON-dosieron (butono)")

#=====================================================================
# 置換用JSONファイルを作成するボタン
#=====================================================================
if st.button("Krei la JSON-dosieron por anstataŭigo"):
    with st.spinner("Konstruas la anstataŭigan JSON-dosieron... Bonvolu atendi iomete."):
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (5) placeholder(占位符)を使い安全に段階置換を行うリストを作成
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (6) 実際に safe_replace する (並列 or 単一)
        #-------------------------------------------------------------
        if use_parallel:
            ...
            for i, j in enumerate(E_stem_with_Part_Of_Speech_list):
                ...
                if i % 1000 == 0:
                    ...
                    progress_text.write(f"{current_count}/{total_items} elementoj prilaborataj...")

            progress_bar.progress(100)
            progress_text.write("La plej longa prilaborado atingis 100% (ankoraŭ 3~4 sekundoj postulataj).")
        ...
        st.success("La konstruo de la anstataŭiga listo finiĝis sukcese!")

        # 最後のダウンロードボタンもファイル名をエスペラント化
        st.download_button(
            label="Elŝuti la finan anstataŭigan liston (kunigitaj 3 JSON-dosieroj)",
            data=download_data,
            file_name="FinfinaAnstataŭigaListo_kunigitaj3JSONdosieroj.json",  # ★ファイル名をエスペラント化
            mime='application/json'
        )