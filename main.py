...
# タイトル部分（GUI表示のみエスペラント語に）
st.title("Anstataŭigi Esperanto-tekston per Kanji-ideogramoj aŭ aldoni HTML-rubrikojn (Pligrandigita versio)")
st.write("---")
...
selected_option = st.radio(
    "Kiel trakti la JSON-dosieron? (Legi la dosieron JSON por anstataŭigo)",
    json_options,
    format_func=lambda x: "Uzi la defaŭltan JSON-dosieron" if x == "デフォルトを使用する" else "Alŝuti dosieron"
)

with st.expander("Elŝuti ekzemplan JSON-dosieron (por anstataŭigo)"):
    ...
    with open(json_file_path, "rb") as file_json:
        btn_json = st.download_button(
            label="Elŝuti ekzemplan JSON-dosieron",
            data=file_json,
            file_name="ekzemplo_JSON_dosiero.json",
...
if selected_option == "デフォルトを使用する":
    ...
    try:
        ...
        st.success("Sukcese ŝargis la defaŭltan JSON-dosieron.")
    except Exception as e:
        st.error(f"Ne eblis ŝargi la defaŭltan JSON-dosieron: {e}")
        st.stop()
else:
    uploaded_file = st.file_uploader(
        "Alŝuti JSON-dosieron (en formato kiu kunigas 3 listojn).json",
        type="json"
    )
    if uploaded_file is not None:
        try:
            ...
            st.success("La alŝutita JSON-dosiero sukcese ŝargitis.")
        except Exception as e:
            st.error(f"Ne eblis legi la alŝutitan JSON-dosieron: {e}")
            st.stop()
    else:
        st.warning("Neniu JSON-dosiero estis alŝutita. Halto.")
        st.stop()
...
# 同時プロセス数の選択
#=================================================================
st.header("Altnivelaj Agordoj (Paralela Pretigo)")
with st.expander("Malfermi agordojn por paralela pretigo"):
    st.write("""
    Ĉi tie vi povas agordi la nombron da procezoj, kiuj rulas samtempe
    dum anstataŭigo de la ideogramoj (kanji).
    """)
    use_parallel = st.checkbox("Uzi paralelan pretigon", value=False)
    num_processes = st.number_input(
        "Kvanto da samtempaj procezoj",
...
options_esperanto_labels = {
    'HTML格式_Ruby文字_大小调整': "HTML-formato kun rubi-anotacioj kaj grandec-ĝustigo",
    'HTML格式_Ruby文字_大小调整_汉字替换': "HTML-formato kun rubi-anotacioj, grandec-ĝustigo kaj kanji-anstataŭigo",
    'HTML格式': "HTML-formato",
    'HTML格式_漢字替换': "HTML-formato kun kanji-anstataŭigo",
    '括弧(号)格式': "Formato kun krampoj",
    '括弧(号)格式_漢字替换': "Formato kun krampoj kaj kanji-anstataŭigo",
    '替换后文字列のみ(仅)保留(简单替换)': "Nur konservi la anstataŭigitan tekston (simpla anstataŭigo)"
}

display_options = list(options.keys())
selected_display = st.selectbox(
    "Elektu la eligan formaton (la saman, kiel en la JSON-dosiero por anstataŭigo):",
    display_options,
    format_func=lambda key: options_esperanto_labels[key]
)
...
st.subheader("Fonto de eniga teksto")
source_option = st.radio(
    "Kiel vi volas provizi la enigan tekston?",
    source_options,
    format_func=lambda x: "Mane tajpi" if x == "手動入力" else "Alŝuti dosieron"
)

uploaded_text = ""

if source_option == "ファイルアップロード":
    text_file = st.file_uploader("Alŝuti tekstan dosieron (UTF-8 enkodado)", type=["txt", "csv", "md"])
    if text_file is not None:
        uploaded_text = text_file.read().decode("utf-8", errors="replace")
        st.info("Sukcese alŝutis la tekstan dosieron.")
    else:
        st.warning("Neniu teksta dosiero estis alŝutita. Bonvolu uzi manan enigon aŭ alŝuti dosieron.")
...
with st.form(key='profile_form'):
    ...
    text0 = st.text_area(
        "Bonvolu enigi la Esperanto-tekston ĉi tie",
        height=150,
        value=initial_text
    )

    st.markdown("""Se vi ĉirkaŭas parton de la teksto per **%** 
    (ekz. `%<teksto ĝis 50 signoj>%`),
    tiu parto **ne estos anstataŭigita** kaj restos senŝanĝa en la fina rezulto.""")

    st.markdown("""Simile, se vi ĉirkaŭas parton de la teksto per **@** 
    (ekz. `@<teksto ĝis 18 signoj>@`),
    tiu parto estos anstataŭigita **loke** (nur ene de tiu fragmento).""")

    letter_type = st.radio(
        'Elektu la prezentan formon de la specifaj Esperanto-literoj en la rezulto',
        ('上付き文字', 'x 形式', '^形式'),
        format_func=lambda x: (
            "Supersigna formo (ĉ → c + ˆ)" if x == "上付き文字"
            else ("x-formato (ĉ → cx)" if x == "x 形式" else "^-formato (ĉ → c^)")
        )
    )

    submit_btn = st.form_submit_button('Sendi')
    cancel_btn = st.form_submit_button("Nuligi")

    if cancel_btn:
        st.warning("Procezo nuligita.")
        st.stop()
...
if processed_text:
    ...
    if len(lines) > MAX_PREVIEW_LINES:
        ...
        preview_text = "\n".join(first_part) + "\n...\n" + "\n".join(last_part)
        st.warning(
            f"La teksto estas tre longa (entute {len(lines)} linioj). "
            "Nur limigita antaŭrigardo estas montrata (la unuaj 247 linioj kaj la lastaj 3 linioj)."
        )
    else:
    ...
    st.download_button(
        label="Elŝuti la rezulton",
        data=download_data,
        file_name="anstatauxigo_rezulto.html",
        mime="text/html"
    )
